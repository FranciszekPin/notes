...
* tracking branch - local branch with connection to remote branch - doing `pull`/`push` will modify upstream pointer
...
print only commits which changed occurance of specified string (for example added or removed reference of some function)
```
...
Fetch changes and merge from remote repo (works only if branch is tracking remote branch)
```
git pull
```
...
Shorthand for shorthand, woking only if local branch (a) doesn’t exist and (b) exactly matches a name on only one remote 
```
git checkout <branch>
```
